predict.maxent <- function(object, feature_matrix, ...) {
  ...
  classify_maxent <- function(feature_matrix, model) {
    ...
    results <- maximumentropy$classify_samples(
      as.integer(feature_matrix@dimension[1]),
      as.integer(feature_matrix@dimension[2]),
      feature_matrix@ia, ja, feature_matrix@ra, model)
  ...
  classify_maxent(feature_matrix, object@model)
}
